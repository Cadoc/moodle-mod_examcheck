...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        ...
        $collection->add_database_table('examcheck_seat_users', [
            'userid'      => 'privacy:metadata:examcheck_seat_users:userid',
            'assignedby'  => 'privacy:metadata:examcheck_seat_users:assignedby',
            'seatid'      => 'privacy:metadata:examcheck_seat_users:seatid',
            'timecreated' => 'privacy:metadata:examcheck_seat_users:timecreated',
        ], 'privacy:metadata:examcheck_seat_users');
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        $sql = "SELECT ctx.id
                  ...
                  JOIN {modules} m ON m.id = cm.module AND m.name = :modname
                  JOIN {examcheck} e ON e.id = cm.instance
                  JOIN {examcheck_seat_users} su ON su.examcheckid = e.id
                 WHERE su.userid = :userid OR su.assignedby = :assignedby";
        $contextlist->add_from_sql($sql, [
            'modlevel'   => CONTEXT_MODULE,
            'modname'    => 'examcheck',
            'userid'     => $userid,
    ...
    public static function get_users_in_context(userlist $userlist): void {
        ...
        $sql = "SELECT su.userid, su.assignedby
                  FROM {course_modules} cm
                  JOIN {modules} m ON m.id = cm.module AND m.name = :modname
                  JOIN {examcheck} e ON e.id = cm.instance
                  JOIN {examcheck_seat_users} su ON su.examcheckid = e.id
        ...
        $userlist->add_from_sql('userid', $sql, $params);
        $userlist->add_from_sql('assignedby', $sql, $params);
    }
    ...
    public static function export_user_data(approved_contextlist $contextlist): void {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            // Seat assignments, exported under their own "seats" subpath.
            $assignments = $DB->get_records_sql(
                "
                SELECT su.id, su.timecreated, su.userid, su.assignedby, s.label
                  FROM {examcheck_seat_users} su
                  JOIN {examcheck_seats} s ON s.id = su.seatid
                 WHERE su.examcheckid = :examcheckid
                   AND (su.userid = :userid OR su.assignedby = :assignedby)
              ORDER BY su.timecreated ASC",
                ['examcheckid' => $cm->instance, 'userid' => $userid, 'assignedby' => $userid]
            );
            ...
            if ($assignments) {
                ...
                foreach ($assignments as $assignment) {
                    $entry = (object) [
                        ...
                        'timecreated' => \core_privacy\local\request\transform::datetime($assignment->timecreated),
                    ];
                    if ((int) $assignment->userid === $userid) {
                        $myseat[] = $entry;
                    }
                    if ((int) $assignment->assignedby === $userid) {
                        $assignedbyme[] = $entry;
                    }
                }
                ...
                writer::with_context($context)->export_data(
                    [get_string('pluginname', 'mod_examcheck'), get_string('privacy:seats', 'mod_examcheck')],
                    $data
                );
            }
        }
    ...
    public static function delete_data_for_all_users_in_context(context $context): void {
        ...
        $DB->delete_records('examcheck_seat_users', ['examcheckid' => $cm->instance]);
    }
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist): void {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            $DB->delete_records('examcheck_seat_users', ['examcheckid' => $cm->instance, 'userid' => $userid]);
            $DB->set_field(
                'examcheck_seat_users',
                'assignedby',
                0,
                ['examcheckid' => $cm->instance, 'assignedby' => $userid]
            );
        }
    }
    ...
    public static function delete_data_for_users(approved_userlist $userlist): void {
        ...
        $DB->set_field_select(
            ...
            'assignedby',
            0,
            "examcheckid = :examcheckid AND assignedby $insql2",
            $params2
        );
    }
}
