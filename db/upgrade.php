...
function xmldb_examcheck_upgrade($oldversion) {
    ...
    if ($oldversion < 2026071001) {
        // Per-activity seats toggle. Existing activities keep seats on; disabling
        // hides and blocks the feature but never deletes seat data.
        $table = new xmldb_table('examcheck');
        $field = new xmldb_field('enableseats', XMLDB_TYPE_INTEGER, '2', null, XMLDB_NOTNULL, null, '1', 'requiresequential');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_mod_savepoint(true, 2026071001, 'examcheck');
    }
