...
final class step_statuses_test extends \advanced_testcase {
    /**
     * The table renders a column per step, checked/unchecked icons, and highlights
    ...
    public function test_renders_columns_icons_and_current_highlight(): void {
        ...
        $html = $OUTPUT->render_from_template('mod_examcheck/scan_result_modal/step_statuses', [
            'steps' => [
                ['name' => 'Attendance', 'checked' => true, 'current' => false],
                ['name' => 'ID verified', 'checked' => false, 'current' => true],
            ],
        ]);

        // A column header per step.
        $this->assertStringContainsString('Attendance', $html);
        $this->assertStringContainsString('ID verified', $html);
        ...
        // The current step column is highlighted.
        $this->assertStringContainsString('examcheck-currentstep', $html);
        $this->assertStringContainsString('table-active', $html);
    }
